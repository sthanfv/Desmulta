...
*   **Sistema Keep-Alive (Wake-Lock) con Anti-Bot Jitter**: Para evitar que Render suspenda las instancias gratuitas de Python y Go por inactividad (evitando el "Cold Start" de 50 segundos), se implementó un **Vercel Cron Job** (`*/14 * * * *`) configurado en `vercel.json` que dispara una petición a `src/app/api/internal/keepalive/route.ts` protegida mediante `CRON_SECRET`.
    *   **Jitter Algorítmico**: Este endpoint genera internamente un retraso (Sleep) aleatorio de hasta 45 segundos antes de lanzar los Pings a Render, evadiendo así las heurísticas de los WAF que detectan y bloquean cron jobs de reloj exacto. Esto garantiza que las máquinas estén calientes 24/7 y la respuesta del OCR caiga siempre a promedios de 2 - 5 segundos.
