...
export function GeminiConsumptionWidget({ requestsToday }: GeminiConsumptionWidgetProps) {
  ...
  return (
    <Card className={`border shadow-lg bg-gradient-to-br from-indigo-50/50 to-white dark:from-indigo-950/20 dark:to-background ${isCritical ? 'border-red-500/50 animate-pulse' : 'border-indigo-500/20'}`}>
      <CardHeader className="pb-2">
        <CardTitle className={`text-sm font-medium flex items-center gap-2 ${isCritical ? 'text-red-600' : 'text-indigo-700 dark:text-indigo-400'}`}>
      ...
      <CardContent>
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1">
            ...
            <p className="text-2xl font-bold">{requestsToday.toLocaleString()} / {DAILY_LIMIT.toLocaleString()}</p>
          </div>

          <div className={`space-y-1 border-l pl-4 border-indigo-100 dark:border-indigo-800/30 ${statusColor}`}>
        ...
        {(isCritical || isWarning) && (
          <div className={`mt-4 pt-3 border-t ${isCritical ? 'border-red-200 dark:border-red-900/50' : 'border-yellow-200 dark:border-yellow-900/50'}`}>
            <p className={`text-xs flex items-center gap-1 ${isCritical ? 'text-red-600 dark:text-red-400 font-bold' : 'text-yellow-600 dark:text-yellow-400'}`}>
