...
- **Qué cambió:**
  ...
  - **[Bugfix CSP Animaciones 3D]**: Se reactivó `'unsafe-inline'` en la directiva `style-src` del Content Security Policy en `src/lib/security-headers.ts`. Esto soluciona un bloqueo severo en producción que impedía a librerías de renderizado 3D y Framer Motion inyectar estilos en línea (los cuales cambian frame por frame y no pueden usar Nonces estáticos).
- **Por qué cambió:**
