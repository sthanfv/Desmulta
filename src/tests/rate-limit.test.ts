...
describe('🛡️ Escudo Anti-DDoS y Rate Limiting', () => {
  ...
  it('🚫 Debe bloquear peticiones masivas y devolver Error 429', async () => {
    ...
    // Debug crítico: Imprime los códigos reales si el test falla
    console.log('HTTP Status Codes:', codigosRespuesta);

    const servidorNoDisponible = codigosRespuesta.length === 0;
    if (servidorNoDisponible) {
      console.warn(
        '⚠️ Test omitido: El servidor 9005 no está respondiendo. Este test requiere npm run dev activo.'
      );
      return;
    }

    expect(codigosRespuesta).toContain(429);
