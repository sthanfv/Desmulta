...
export function getLevenshteinDistance(a: string, b: string): number {
  ...
  for (let i = 1; i <= m; i++) {
    for (let j = 1; j <= n; j++) {
      // Usamos el operador asertivo no nulo (!) ya que inicializamos la matriz rígidamente antes
      const cost = a[i - 1]?.toLowerCase() === b[j - 1]?.toLowerCase() ? 0 : 1;

      const top = matrix[i - 1]![j]! + 1; // Eliminación

      const left = matrix[i]![j - 1]! + 1; // Inserción
      const diag = matrix[i - 1]![j - 1]! + cost; // Sustitución

      matrix[i]![j] = Math.min(top, left, diag);
    }
  }
