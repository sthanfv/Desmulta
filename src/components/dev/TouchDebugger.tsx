...
interface TouchLog {
  ...
  y: number;
  tagName: string;
  /** Duración del toque en ms (touchstart → touchend) */
  durationMs?: number;
}

/** Registro de un evento RAGE_CLICK detectado */
...
export function TouchDebugger() {
  ...
  /**
   * touchLogs vive en un ref para evitar re-renders en cada toque mientras se
   * depura la aplicación. Solo se lee al cambiar al tab TOUCH.
   */
  const touchLogsRef = useRef<TouchLog[]>([]);
  /** Snapshot inmutable del ref para el render — solo se actualiza cada N toques para no causar re-renders en cada uno */
  const [touchLogsSnapshot, setTouchLogsSnapshot] = useState<TouchLog[]>([]);
  /** Conteo de logs visibles — estado separado para evitar leer el ref durante el render */
  const [touchLogsCount, setTouchLogsCount] = useState(0);

  /** Estado del último RAGE_CLICK detectado */
  const [rageClick, setRageClick] = useState<RageClickEntry | null>(null);
  /** Historial de toques por selector de elemento para detección RAGE_CLICK */
  const elementTapHistory = useRef<Map<string, number[]>>(new Map());
  /** Timestamp del touchstart activo para medir duración */
  const activeTouchStart = useRef<{ id: number; time: number; tagName: string } | null>(null);
  ...
  const activatePanel = useCallback(() => {
    ...
    // Limpiar logs táctiles al abrir el panel
    touchLogsRef.current = [];
    setTouchLogsSnapshot([]);
    setTouchLogsCount(0);
    elementTapHistory.current.clear();
    setRageClick(null);
    setIsActive(true);
    setTapCount(0);
  }, []);
  ...
  const handleDebugTouch = useCallback(
    (e: TouchEvent) => {
      ...
      // Guardar en ref (sin re-render por cada toque)
      touchLogsRef.current = [newLog, ...touchLogsRef.current.slice(0, 29)];
      activeTouchStart.current = { id: logId, time: now, tagName };
      const nuevoConteo = touchLogsRef.current.length;

      // ── Detección RAGE_CLICK ────────────────────────────────────────────
      // Construir clave única de elemento: tagName + aria-label o id si existen
      const elemId = target?.id || target?.getAttribute('aria-label') || tagName;
      const historia = elementTapHistory.current.get(elemId) || [];
      const historiaCortada = [...historia.filter((t) => now - t < 2000), now];
      elementTapHistory.current.set(elemId, historiaCortada);

      if (historiaCortada.length >= 5) {
        const entrada: RageClickEntry = {
          elemento: elemId,
          conteo: historiaCortada.length,
          x: Math.round(touch.clientX),
          y: Math.round(touch.clientY),
          timestamp: new Date().toISOString().split('T')[1].slice(0, 11),
        };
        setRageClick(entrada);
        elementTapHistory.current.set(elemId, []); // reiniciar contador

        // Reportar a Sentry con contexto del elemento
        Sentry.withScope((scope) => {
          scope.setLevel('warning');
          scope.setTag('tipo', 'RAGE_CLICK');
          scope.setExtra('elemento', elemId);
          scope.setExtra('conteo', entrada.conteo);
          scope.setExtra('coordenadas', `${entrada.x}, ${entrada.y}`);
          Sentry.captureMessage('[TouchDebugger] RAGE_CLICK detectado');
        });
      }

      // Actualizar estado React para que el render vea los nuevos logs
      // Solo cuando el tab TOUCH está activo (evitar re-renders innecesarios)
      setTouchLogsCount(nuevoConteo);
      if (activeTab === 'TOUCH') {
        setTouchLogsSnapshot([...touchLogsRef.current]);
      }
    },
    [isActive, activeTab]
  );

  // ── Captura de touchend para medir duración del toque ─────────────────────
  const handleDebugTouchEnd = useCallback(
    (e: TouchEvent) => {
      if (!isActive || activeTab !== 'TOUCH') return;
      if (!activeTouchStart.current) return;
      const duracionMs = Date.now() - activeTouchStart.current.time;
      // Actualizar el log con la duración real del toque
      const idx = touchLogsRef.current.findIndex((l) => l.id === activeTouchStart.current!.id);
      if (idx !== -1) {
        touchLogsRef.current[idx] = { ...touchLogsRef.current[idx], durationMs: duracionMs };
        setTouchLogsSnapshot([...touchLogsRef.current]);
      }
      activeTouchStart.current = null;
      void e; // suprimir advertencia de parámetro no usado
    },
    [isActive, activeTab]
  );
  ...
  const refreshTab = useCallback((tab: DebugTab) => {
    ...
    if (tab === 'TOUCH') setTouchLogsSnapshot([...touchLogsRef.current]);
    if (tab === 'DEVICE') {
  ...
  const getFormattedLogText = () => {
    ...
    if (activeTab === 'TOUCH') {
      body += touchLogsSnapshot
        .map(
          (l) =>
            `[${l.time}] X:${l.x} Y:${l.y} Elemento: <${l.tagName}>${l.durationMs !== undefined ? ` (${l.durationMs}ms)` : ''}`
        )
        .join('\n');
  ...
  useEffect(() => {
    ...
    document.addEventListener('touchend', handleDebugTouchEnd, { passive: true });
    ...
    return () => {
      ...
      document.removeEventListener('touchend', handleDebugTouchEnd);
      clearInterval(interval);
    };
  }, [isActive, handleDebugTouch, handleDebugTouchEnd, activeTab]);
  ...
  const TABS: { id: DebugTab; label: string; count?: number; rageBadge?: boolean }[] = [
    ...
    { id: 'TOUCH', label: 'TOUCH', count: touchLogsCount, rageBadge: rageClick !== null },
    ...
    >
      ...
      <div className="flex flex-wrap items-stretch border-b border-white/10 select-none bg-zinc-900/50 shrink-0">
        {TABS.map((tab) => (
          ...
          >
            ...
            {tab.rageBadge && (
              <span
                className="absolute top-0.5 left-0.5 w-2 h-2 rounded-full bg-red-500 animate-pulse"
                aria-label="RAGE_CLICK detectado"
              />
            )}
            {!tab.rageBadge && tab.count !== undefined && tab.count > 0 && (
              <span
      ...
      <div className="overflow-y-auto flex-1 select-text scrollbar-thin scrollbar-thumb-white/10">
        ...
        {(activeTab === 'MEDIA' || activeTab === 'ERRORES' || activeTab === 'TOUCH') && (
          <div className="p-1">
            {/* Banner RAGE_CLICK — solo tab TOUCH */}
            {activeTab === 'TOUCH' && rageClick && (
              <div className="flex items-center gap-2 mx-1 mb-2 px-2 py-1.5 rounded-lg bg-red-900/40 border border-red-500/40">
                <span className="w-2 h-2 rounded-full bg-red-400 animate-pulse flex-shrink-0" />
                <span className="text-[9px] text-red-300 font-black uppercase tracking-wider">
                  RAGE_CLICK · {rageClick.elemento} · {rageClick.conteo}x ({rageClick.x},
                  {rageClick.y}) {rageClick.timestamp}
                </span>
                <button
                  onClick={() => setRageClick(null)}
                  className="ml-auto text-[8px] text-red-400 hover:text-red-200"
                >
                  ✕
                </button>
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      {tl.durationMs !== undefined && (
                        <span
                          className={`text-[8px] font-bold ml-1 ${tl.durationMs > 500 ? 'text-amber-400' : 'text-white/30'}`}
                        >
                          {tl.durationMs}ms
                        </span>
                      )}
