...
| v1.0.0  | 🟢 Estable | **Hotfix TypeScript (Webhook Telegram):** Se corrigió un error de compilación estricto (`TS2339`) introducido en el parche anterior al acceder a `reply_markup` del objeto `cb.message`. Se aplicó un *type assertion* explícito (`as any`) para sortear la restricción de la interfaz y permitir el despliegue exitoso en Firebase Functions. |
