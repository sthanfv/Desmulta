...
export function Lightbox({ src, alt, className = '', priority = false, blurDataURL }: LightboxProps) {
  ...
  const triggerContent = (
    <div className={`relative group cursor-zoom-in overflow-hidden w-full ${className}`}>
      {/* Imagen miniatura */}
      <Image
        src={src}
        alt={alt}
      ...
      {mounted && (
        <DialogContent className="max-w-7xl w-[95vw] h-[95vh] p-0 bg-transparent border-none shadow-none flex flex-col justify-center items-center">
          ...
          <DialogPrimitive.Description className="sr-only">
            Visualización detallada de evidencia técnica, captura de pantalla o testimonio.
