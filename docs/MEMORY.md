...
| :------ | :--------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v1.0.0  | 🟢 Estable | **Remediación Auditoría Forense Frontend:** Se resolvieron 8 hallazgos identificados en auditoría estricta. (1) Rate Limit en `/api/auth/verify-otp` previene fuerza bruta de OTP. (2) Sincronización `localStorage/sessionStorage` en `ReturningUserBanner` evita 404s y posibles IDORs. (3) `estado/actions.ts` oculta trazas de error internas al cliente. (4) `vercel.json` implementa CSP/HSTS para assets estáticos. (5) `TouchDebugger` ahora valida hashes criptográficos del PIN en lugar de compararlo en texto plano, protegiendo contra extracción en cliente. (6) Rate Limit añadido a la verificación de estado de pago. (7) Eliminación de `console.error` con metadata sensible en middleware.                                                                                                                                                                          |
| v1.0.0  | 🟢 Estable | **Resolución de Bloqueo Crítico (Telegram Bot No Funciona):** Se identificó que tras el Fix H-14 (Degraded Mode), la variable de entorno `TELEGRAM_SECURITY_CHAT_ID` se añadió en `.env` pero NUNCA se inyectó en Firebase Functions. En consecuencia, el arreglo de control de acceso (`allowedIds`) evaluaba a un arreglo vacío (`[]`), bloqueando silenciosa y permanentemente el 100% de las peticiones (comandos y botones inline) con un status HTTP 200 OK hacia Telegram. **Solución:** Se crearon los secretos en Google Cloud Secret Manager (`firebase functions:secrets:set`) y se añadieron explícitamente (`TELEGRAM_CHAT_ID`, `TELEGRAM_DEV_CHAT_ID`, `TELEGRAM_SECURITY_CHAT_ID`) al array de dependencias de secretos en `telegramWebhook.ts` para que se monten en tiempo de ejecución.                                                                               |
| v1.0.0  | 🟢 Estable | **Corrección Telegram Webhook (Bug Ver Cédula en Estado):** Se identificó un bug silencioso: cuando el operador solicitaba ver la cédula sobre un mensaje modificado de "ESTADO ACTUALIZADO" (que no contiene la etiqueta de cifrado original), el bot mostraba el pop-up de éxito pero no revelaba nada al no encontrar la etiqueta. **Solución:** Se implementó una lógica condicional (fall-back) que detecta si el texto original no muta; si es así, se anexa dinámicamente (\n\n🪪 **Cédula revelada:** XXXX) al pie del mensaje editado.                                                                                                                                                                                                                                                                                                                                         |
| v1.0.0  | 🟢 Estable | **Hotfix TypeScript (Webhook Telegram):** Se corrigió un error de compilación estricto (`TS2339`) introducido en el parche anterior al acceder a `reply_markup` del objeto `cb.message`. Se aplicó un _type assertion_ explícito (`as any`) para sortear la restricción de la interfaz y permitir el despliegue exitoso en Firebase Functions.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
## 📝 SESIÓN: RESOLUCIÓN DE BUGS CRÍTICOS (Agosto 2026)

**Objetivo:** Restaurar el acceso al panel de administración y corregir bugs de contraste visual.

**Cambios e Implementaciones:**

- **Resolución Bug 401 (Acceso Denegado):** Se identificó que el `idToken` provisto por Firebase Auth poseía un UID nuevo tras una recreación manual de la cuenta, mientras que la colección `admins` de Firestore mantenía el UID antiguo. El sistema (Zero-Trust) repelió correctamente el acceso (Status 401). Se resolvió insertando el nuevo UID directamente en la colección de producción sin necesidad de modificar el código.
- **Corrección de Accesibilidad Visual (`src/app/admin/auditoria/page.tsx`):** Se solucionó la falla de legibilidad en el _toast_ de Modo Dios. El wrapper de la notificación aplicaba clases de Glassmorphism claras (`bg-white/90`) sobre la capa de fondo esmeralda. Se ajustaron las clases (`[&>div]:bg-emerald-50` y `text-emerald-900`) para forzar un alto contraste.

**Estado Arquitectónico:**

- 🟢 Estable. La validación completa de lint y tipado pasó de forma limpia. El acceso a los administradores se encuentra restaurado.

## 📝 SESIÓN: AUDITORÍA UX Y OPTIMIZACIÓN SEO (Zero-Cost) (Agosto 2026)

**Objetivo:** Optimizar los indicadores Core Web Vitals (FCP y LCP) para posicionamiento orgánico gratuito en móviles, además de documentar arquitectónicamente el motor de generación de rutas estáticas SEO ya existente.

**Cambios e Implementaciones:**

- **Documentación de Arquitectura SEO (`docs/SEO_ARCHITECTURE.md`):** Se creó el documento maestro que detalla la política de cero JavaScript bloqueante, el enrutamiento de Next.js (SSG) de ciudades, el uso de JSON-LD Schema.org y las directrices de SEO estático.
- **Optimización UX Móvil (`src/components/ui/MeshBackground.tsx`):** Se inyectó CSS condicional (`md:hidden`) y un degradado nativo (`bg-gradient-to-br`) para desactivar el renderizado en tiempo real de `framer-motion` en teléfonos móviles. Esto mitiga permanentemente el _Scroll Jank_ en Android gama media y protege la batería, reduciendo el FCP (First Contentful Paint).
- **Optimización LCP en Imagen Principal (`src/components/sections/Hero.tsx`):** Se eliminó el contenedor con retraso `animate-in zoom-in-95 duration-1000` de la imagen del _Hero_, permitiendo que la etiqueta `<Lightbox>` (basada en `next/image` con `priority={true}`) pinte la imagen inmediatamente tras la resolución del DOM. Esto evita penalizaciones de Google en el Largest Contentful Paint.

**Estado Arquitectónico:**

- 🟢 Estable. La validación completa (`npm run validate`) pasó todos los tests de integración, E2E, lint y tipeo estricto.
...
- **Wrapper de Caché Redis (`src/lib/cache/redis-cache.ts`):** Creación del helper `getCachedDoc` y `invalidateCache` sobre Upstash Redis. Aplica una política _fail-safe_: si Redis falla o no responde, cae de forma segura a Firestore sin lanzar errores al cliente final.
...
- **Interlinking de SEO en Footer (`Footer.tsx`)**:
  - Se implementó una sección de siloing de enlaces en el componente de pie de página global [Footer.tsx](file:///c:/Workspace/Desmulta/src/components/sections/Footer.tsx) titulada _"Defensa por Código de Multa"_.
...
- **Feedback Visual In-App**:
  ...
  - Cuando se copia el enlace en PC, se muestra un badge animado sumamente estético y de color esmeralda al pie de las historias que notifica instantáneamente: _"¡Enlace copiado al portapapeles! 📋"_ durante 2.5 segundos (animado con Framer Motion y `<AnimatePresence>`).
- **QA e Integración**:
...
- **Protección con Circuit Breaker:** Se integró `OcrCircuitBreakerFs` en el flujo de `ocr/route.ts` para proteger las llamadas a Google Gemini AI. Adicionalmente, se protegió la conexión raíz `getAdminApp()` en `firebase-admin.ts` con el `FirebaseCircuitBreaker` _en memoria_, respetando el estándar de la industria para entornos serverless (evitando la paradoja de usar Firestore para validar Firestore).
...
- **[Filtro de Relevancia Heurístico]:** Incorporada una lista negra de exclusión en [sync-blog-rss.ts](file:///c:/Workspace/Desmulta/scripts/sync-blog-rss.ts) (palabras como _"choque"_, _"fallecido"_, _"herido"_, _"accidente"_) para descartar de forma automática reportes trágicos o colisiones viales comunes que suelen contaminar los feeds de transporte, manteniendo el blog enfocado en regulaciones y multas.
