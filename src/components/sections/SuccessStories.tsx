...
export function SuccessStories() {
  ...
  const storiesQuery = useMemoFirebase(() => {
    ...
    return query(collection(firestore, 'success_cases'), orderBy('createdAt', 'desc'), limit(4));
  }, [firestore]);
  ...
  return (
    <section className="py-24 px-4 bg-background relative overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
          <div className="max-w-2xl">
            <h2 className="text-3xl md:text-5xl font-black text-foreground mb-6 tracking-tight leading-none uppercase flex flex-col gap-2">
            ...
            <p className="text-muted-foreground text-lg font-medium leading-relaxed">
              ...
              análisis técnico de alta precisión. La transparencia en los resultados es nuestra
              mayor garantía.
            </p>
          </div>
          ...
          <LazyMotion features={domAnimation}>
            <AnimatePresence mode="popLayout">
              {displayStories.map((story, index) => (
                ...
                >
                  <MagneticCard
                    className={`p-8 rounded-[2rem] group transition-all duration-500 h-full hover:scale-[1.02] ${isLive ? 'border-primary/20 hover:border-primary border-2 shadow-[0_0_15px_rgba(255,193,7,0.05)] hover:shadow-[0_0_30px_rgba(255,191,0,0.15)] bg-gradient-to-b from-card/80 to-primary/5' : 'hover:border-primary/50'}`}
                  >
                    {/* Live Indicator Dot */}
                    {isLive && (
                    ...
                    <div className="flex justify-between items-start mb-8 relative z-10">
                      <div className="flex flex-col ml-3">
                        ...
                        <span className="text-[8px] font-bold text-muted-foreground/80 uppercase tracking-tighter">
                    ...
                    <div className="space-y-6 relative z-10">
                      <div>
                        ...
                        <p
                          className={`text-2xl sm:text-3xl font-black italic transition-colors duration-300 ${isLive ? 'text-white group-hover:text-primary drop-shadow-md' : 'text-foreground'}`}
                        >
                          {story.amount}
                        </p>
                      </div>
                      ...
                      {/* MANDATO-FILTRO v7.4.3: Botón explícito con target táctil ≥44px (WCAG 2.1).
                        Antes: toda la tarjeta era un mega-botón invisible que abría el modal,
                        causando toques fantasma en Android. Ahora solo este botón dispara la acción. */}
                      <div className="flex justify-between items-center">
                        <div className="inline-flex px-3 py-1 rounded-lg bg-white/5 border border-white/10 text-[9px] font-black uppercase tracking-widest text-foreground/50 text-wrap leading-tight max-w-[160px] md:max-w-none">
                          {story.type}
                        </div>
                        <button
                          type="button"
                          ...
                          className="min-h-[44px] min-w-[44px] px-4 py-2 rounded-xl bg-primary/10 hover:bg-primary/20 active:scale-95 transition-all text-[10px] font-black text-primary uppercase tracking-widest border border-primary/20 hover:border-primary/40 flex items-center gap-1.5"
                        >
                          CONSULTAR
                    ...
                    {/* Decorative accent */}
                    <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 blur-3xl rounded-full -mr-16 -mt-16 group-hover:bg-primary/20 transition-colors duration-500 pointer-events-none" />
                  </MagneticCard>
                </m.div>
              ))}
            </AnimatePresence>
        ...
        <div className="mt-12 text-center reveal">
          <p className="text-[11px] text-muted-foreground font-medium max-w-2xl mx-auto leading-relaxed opacity-60">
            * Por protocolos de Seguridad Técnica y cumplimiento de la Ley 1581 de 2012 (Protección
