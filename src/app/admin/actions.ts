 ...
 * Evita el uso de 'any' manteniendo el narrowing de tipo seguro.
 */
...
/**
 * Guarda de tipo que verifica si un valor es un Timestamp de Firestore.
 */
function isFirestoreTimestamp(obj: unknown): obj is FirestoreTimestampLike {
  return (
    obj !== null &&
    typeof obj === 'object' &&
    'toDate' in obj &&
    typeof (obj as FirestoreTimestampLike).toDate === 'function'
  );
}

/**
 * Helper de serialización estricta.
 * Next.js Server Components lanzan error ("An error occurred in the Server Components render")
 * si se retornan objetos complejos como Timestamps o Referencias de Firestore.
 */
function serializeDataForNextJS(obj: unknown): unknown {
  if (obj === null || obj === undefined) return obj;
  if (isFirestoreTimestamp(obj)) {
    return obj.toDate().toISOString();
  }
  if (Array.isArray(obj)) {
    return obj.map(serializeDataForNextJS);
  }
  if (typeof obj === 'object') {
    const res: Record<string, unknown> = {};
    for (const key of Object.keys(obj as Record<string, unknown>)) {
      res[key] = serializeDataForNextJS((obj as Record<string, unknown>)[key]);
    }
    return res;
  }
  return obj;
...
const getCachedAnalyticsStats = unstable_cache(
  async () => {
    ...
    // ── Lecturas Optimizadas (Contadores Distribuidos) ─────────────────────────
    // Reducción masiva de lecturas (De ~8,000 a < 20 lecturas)
    ...
    // Agrupar métricas globales
    let totalLeads = 0;
    const statusMap = new Map<string, number>();
    const infractionMap = new Map<string, number>();

    globalMetricsSnap.forEach((doc) => {
      const data = doc.data();
      totalLeads += data.totalConsultations || 0;

      // Agrupar estados e infracciones (Keys: status_XYZ, infraction_XYZ)
      for (const [key, value] of Object.entries(data)) {
        if (typeof value === 'number') {
          if (key.startsWith('status_')) {
            const status = key.replace('status_', '').toUpperCase();
            statusMap.set(status, (statusMap.get(status) || 0) + value);
          } else if (key.startsWith('infraction_')) {
            const infraction = key.replace('infraction_', '');
            infractionMap.set(infraction, (infractionMap.get(infraction) || 0) + value);
          }
        }
      }
    });
    ...
    // Agrupar métricas diarias (Crecimiento de 7 días y Embudo de 30 días)
    ...
    dailyMetricsSnap.forEach((doc) => {
      ...
      if (date >= sevenDaysAgoStr && growthMap.has(date)) {
        growthMap.set(date, (growthMap.get(date) || 0) + (data.consultations || 0));
      }
    });
    ...
    // Para el tiempo promedio, descargamos solo un subconjunto ligero o usamos aproximaciones.
    // Como las métricas de rendimiento deben ser eficientes, sacamos los últimos 50 casos finalizados.
    ...
    try {
      ...
      const resolvedCases = casesSnap.docs;
