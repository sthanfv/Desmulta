        ...
        {!isSimitMode && (
          <>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="space-y-2">
                  <div className="flex items-center gap-3 p-3 rounded-2xl bg-primary/5 border border-primary/20 mb-1">
                    <FormField
                      control={form.control}
                      name="requiresOperatorFiling"
                      render={({ field: checkField }) => (
                        <>
                          <input
                            id="requires-operator-checkbox"
                            type="checkbox"
                            className="w-5 h-5 accent-primary rounded cursor-pointer"
                            checked={checkField.value as boolean}
                            onChange={(e) => checkField.onChange(e.target.checked)}
                          />
                          <label
                            htmlFor="requires-operator-checkbox"
                            className="text-xs font-bold text-foreground cursor-pointer leading-tight"
                          >
                            Que el operador radique por mí
                          </label>
                        </>
                      )}
                    />
