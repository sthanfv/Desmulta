...
export function ConsultationsList() {
  ...
  const fetchConsultations = useCallback(
    async (cursor?: string | null) => {
      ...
      const result = await getConsultations(idToken, 20, cursor);
  ...
  const handleConvertToCase = async (lead: Consultation) => {
    ...
    try {
      ...
      const idToken = (await auth?.currentUser?.getIdToken(true)) || '';
      const result = await convertToCase(idToken, lead);
      if (result.success) {
  ...
  const handleUpdateStatus = async (id: string, status: string) => {
    try {
      ...
      const idToken = (await auth?.currentUser?.getIdToken(true)) || '';
      const result = await updateConsultationStatus(idToken, id, status);
      if (result.success) {
        setConsultations((prev) =>
