...
*   **Microservicio OCR de Respaldo (Lector-OCR)**: Se construyó desde cero una API externa en Python 3.11 (`C:\Workspace\Lector-OCR`) usando FastAPI y PyTesseract para procesar imágenes del SIMIT directamente a nivel del OS mediante C++, ahorrando inmensas cantidades de memoria RAM en comparación con `tesseract.js`. Extrae array de comparendos, placa, cédula y valores usando Heurísticas (Regex).
...
*   **Pre-warming OCR (Cold Start Mitigation)**: Para combatir los 50 segundos de inactividad (Cold Start) del plan gratuito de Render, se añadió un endpoint `GET /api/ocr` en Vercel que actúa como proxy y hace un ping a `/health` en Render. Este endpoint es llamado silenciosamente por `OCRPrewarmer.tsx` 5 segundos después de que el usuario carga la página inicial, garantizando que cuando el usuario llegue al paso de subir la fotomulta, el contenedor de Python ya esté despierto y listo.
