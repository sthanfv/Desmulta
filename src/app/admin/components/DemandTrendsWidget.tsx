      ...
      <CardContent className="grid grid-cols-1 md:grid-cols-12 gap-8 pt-4 relative z-10">
        ...
        <div className="md:col-span-7 space-y-5">
          ...
          {data?.topics.length === 0 ? (
            <div className="h-32 flex items-center justify-center border border-dashed border-border/50 rounded-lg">
              <p className="text-sm text-muted-foreground">Esperando telemetría del mes actual...</p>
            </div>
          ) : (
                ...
                <div key={topic.id} className="space-y-1.5">
                  <div className="flex justify-between text-sm items-end">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono font-bold text-foreground">{topic.percentage}%</span>
                    </div>
                  </div>
        ...
        <div className="md:col-span-5 flex flex-col justify-between space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              {data?.cities.length === 0 ? (
                <div className="h-24 col-span-2 flex items-center justify-center border border-dashed border-border/50 rounded-lg">
                  <p className="text-sm text-muted-foreground col-span-2">Sin geocontexto reciente</p>
                </div>
              ) : (
                data?.cities.map((city, idx) => (
                  <div key={idx} className="bg-secondary/40 p-3 rounded-xl border border-border/40 flex flex-col justify-center items-center hover:bg-secondary/60 transition-colors">
                    <span className="text-lg font-black text-foreground/90">{city.city}</span>
                    <span className="text-xs text-muted-foreground font-medium">{city.percentage}% de consultas</span>
                  </div>
                ))
              )}
            </div>
          ...
          <div className="p-5 bg-gradient-to-br from-amber-500/10 to-amber-500/5 border border-amber-500/20 rounded-2xl flex items-center justify-between shadow-inner">
            <div>
              <p className="text-xs font-bold text-amber-500/80 uppercase tracking-wider mb-1">Volumen Analizado</p>
              <div className="flex items-baseline gap-2">
                <h3 className="text-4xl font-black text-amber-500 tracking-tighter">{data?.totalQueries}</h3>
