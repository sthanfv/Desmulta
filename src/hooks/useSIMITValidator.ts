...
export const useSIMITValidator = () => {
  ...
  const validarImagenSIMIT = async (archivo: File): Promise<ResultadoOCR> => {
    ...
    try {
      ...
      // Un canvas de 1000px procesa al doble de velocidad que 1600px en Tesseract.
      try {
        mediaLogger.log('COMPRESSION', 'Iniciando Droid-Fix (Protección de RAM)...');
        archivoProcesar = await comprimirCaptura(archivo, 1000, 0.9);
      ...
      try {
        ...
        if (esTimeout) {
          return {
            esValida: false, // NO Fail-Open
            coincidencias: [],
            ocrOmitido: true,
            multasExtraidas: [],
            palabrasDetectadas: [],
            error: 'El análisis tomó demasiado tiempo. Por favor intenta con una foto más nítida.',
          };
        }
        throw ocrError;
      } finally {
