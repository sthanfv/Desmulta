...
export async function dispatchToTelegram(
  ...
  try {
    ...
    formData.append('document', pdfBlob, `Poder_Desmulta_${pdfPayload.shortId}.pdf`);

    // Mensaje de contexto para el operador con enmascaramiento de placa (F-15)
    const placaMask =
      pdfPayload.licensePlate && pdfPayload.licensePlate !== 'N/A'
        ? `***${pdfPayload.licensePlate.slice(-2)}`
        : 'N/A';
