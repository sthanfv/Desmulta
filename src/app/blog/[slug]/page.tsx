...
export async function generateMetadata({
  ...
  if (!post) {
    return { title: 'Artículo no encontrado | Desmulta' };
  }

  return {
    title: `${post.meta.title} | Desmulta`,
    description: post.meta.excerpt,
    openGraph: {
      title: post.meta.title,
      description: post.meta.excerpt,
      type: 'article',
      url: `https://desmulta.online/blog/${slug}`,
      images: [
        {
          url: `https://desmulta.online/api/og?title=${encodeURIComponent(post.meta.title)}&type=blog`,
          width: 1200,
          height: 630,
          alt: post.meta.title,
        },
    ...
    twitter: {
      ...
      title: post.meta.title,
      description: post.meta.excerpt,
      images: [`https://desmulta.online/api/og?title=${encodeURIComponent(post.meta.title)}&type=blog`],
    },
  };
}
    ...
    <div className="min-h-screen bg-background relative overflow-hidden selection:bg-primary/30">
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            publisher: {
              ...
              logo: {
                '@type': 'ImageObject',
                url: 'https://desmulta.online/icon-512x512.png',
              },
            },
          }),
        }}
      ...
      <footer className="py-20 text-center opacity-40">
        <p className="text-[10px] font-black uppercase tracking-[0.3em]">
          Guía Legal — Desmulta Tecnologías v1.0.0
        </p>
      </footer>
    </div>
  );
