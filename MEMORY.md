...
*   **Pre-warming OCR (Cold Start Mitigation)**: Para combatir los 50 segundos de inactividad (Cold Start) del plan gratuito de Render, se añadió un endpoint `GET /api/ocr` (y `GET /api/public/calcular-multa`) en Vercel que actúa como proxy y hace pings ligeros. Estos endpoints son llamados silenciosamente por el componente global `<OCRPrewarmer />` en `layout.tsx` 5 segundos después de que el usuario carga la página inicial.
